...
class Indicators {
    ...
    public static function calculateIchimokuCloud($highs, $lows, $closes, $conversionPeriod = 9, $basePeriod = 26, $spanPeriod = 52) {
        ...
        $tenkan_sen = self::calculateEMA(array_slice($highs, -$conversionPeriod), $conversionPeriod) + 
                      self::calculateEMA(array_slice($lows, -$conversionPeriod), $conversionPeriod);
        $tenkan_sen /= 2;

        // محاسبه Kijun-sen (خط پایه)
        $kijun_sen = self::calculateEMA(array_slice($highs, -$basePeriod), $basePeriod) + 
                     self::calculateEMA(array_slice($lows, -$basePeriod), $basePeriod);
        $kijun_sen /= 2;
        ...
        $span_b = self::calculateEMA(array_slice($highs, -$spanPeriod), $spanPeriod) + 
                  self::calculateEMA(array_slice($lows, -$spanPeriod), $spanPeriod);
        $span_b /= 2;

        return ['tenkan_sen' => $tenkan_sen, 'kijun_sen' => $kijun_sen, 'span_a' => $span_a, 'span_b' => $span_b];
    }
    ...
    public static function calculateSuperTrend($highs, $lows, $closes, $atrPeriod = 14, $multiplier = 3) {
        $atr = self::calculateATR($highs, $lows, $closes, $atrPeriod);
        if ($atr === null) {
            return null; // داده کافی نداریم
        }

        // محاسبه SuperTrend
        $upwardTrend = $closes[count($closes) - 1] + ($multiplier * $atr);
        $downwardTrend = $closes[count($closes) - 1] - ($multiplier * $atr);

        return ['upwardTrend' => $upwardTrend, 'downwardTrend' => $downwardTrend];
    }
