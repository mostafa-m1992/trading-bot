...
require_once '/root/tradeBot/trading-bot/NobitexAPI.php';
...
$nobitexAPI = new NobitexAPI($api_key);
...
// دریافت موجودی حساب
$balanceData = $nobitexAPI->getBalance();
...
while (true) {
    try {
        ...
        // تلاش برای خرید
        $tradeResult = $nobitexAPI->placeOrder($symbol, 'buy', $balanceAmount, $price, $orderType);
        ...
        while (!isset($tradeResult)) {
            ...
            $trailingStopResult = $api->manageTrailingStop($symbol, $orderId, $price, $tradedAmount, $trailing_stop);
            ...
            $api->checkAndSell($symbol, $price, $tradedAmount);
            } catch (Exception $th) {
