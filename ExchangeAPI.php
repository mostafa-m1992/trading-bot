...
class ExchangeAPI {
    ...
    public function findBestPair() {
        ...
        // بررسی کلیدهای داده‌ها
        // var_dump(array_keys($pairsData)); // بررسی کلیدها

        // پیدا کردن جفت ارزهای با مبنای تتر
        $pairs = [];
        foreach ($pairsData as $pair => $data) {
            // بررسی اینکه آیا جفت ارز با 'USDT' شروع می‌شود و بعد از آن ارز دیگری وجود دارد
            // شرط دقیق‌تر برای شناسایی جفت ارزهای شروع شونده با USDT
            if (strpos($pair, 'USDT') === strlen($pair) - 4) {
                $pairs[] = $pair;
            }
        }
        // var_dump($pairs);  // چاپ جفت ارزهای پیدا شده

        //  استخراج جفت‌ارزها بر مبنای ریال(بدون 'status')
        ...
        // foreach ($pairsData as $pair => $data) {
        //     // بررسی اینکه آیا جفت ارز با 'USDT' شروع می‌شود و بعد از آن ارز دیگری وجود دارد
        //     // همچنین چک می‌کنیم که 'USDTIRT' نباشد
        //     if (substr($pair, 0, 4) === 'USDT' && strlen($pair) > 4 && $pair !== 'USDTIRT') {
        //         $pairs[] = $pair;
        //     }
        // }
        // var_dump($pairs);  // چاپ جفت ارزهای پیدا شده
        ...
        // foreach ($pairsData as $pair => $data) {
        //     // بررسی اینکه آیا جفت ارز با 'USDT' شروع می‌شود و بعد از آن ارز دیگری وجود دارد
        //     if (substr($pair, 0, 4) === 'USDT' && strlen($pair) > 4) {
        //         $pairs[] = $pair;
        //     }
        // }
        // var_dump($pairs);  // چاپ جفت ارزهای پیدا شده
        ...
        // $pairs = [];
        // foreach ($pairsData as $pair => $data) {
        //     // بررسی اینکه آیا جفت ارز با 'USDT' شروع می‌شود
        //     if (strpos($pair, 'USDT') === 0) {  // جستجو برای 'USDT' در ابتدای نماد
        //         $pairs[] = $pair;
        //     }
        // }
        // var_dump($pairs);  // چاپ جفت ارزهای پیدا شده
        ...
        // $pairs = [];
        // foreach ($pairsData as $pair => $data) {
        //     // بررسی اینکه آیا جفت ارز با 'USDT' تمام می‌شود و قبل از آن ارز دیگری وجود داشته باشد
        //     if (substr($pair, -4) === 'USDT' && strlen($pair) > 4) {
        //         $pairs[] = $pair;
        //     }
        // }
        // var_dump($pairs);  // چاپ جفت ارزهای پیدا شده
        ...
        // $pairs = [];
        // foreach ($pairsData as $pair => $data) {
        //     // بررسی اینکه آیا جفت ارز با 'USDT' تمام می‌شود
        ...
        // $pairs = [];
        // foreach ($pairsData as $pair => $data) {
        //     // بررسی اینکه آیا نام جفت ارز شامل 'usdt' در ابتدا باشد
        //     if (strpos($pair, 'usdt') === 0) {  // اگر 'usdt' در ابتدای نام جفت ارز باشد
        //         $pairs[] = $pair;
        //     }
        // }
        // var_dump($pairs);
        ...
        //     // فرض بر این است که داده‌ها در قالب یک آرایه هستند و می‌خواهیم بررسی کنیم که آیا 'usdt' در یکی از مقادیر باشد
        //     if (is_array($pairData) && in_array('usdt', $pairData)) {
        //         $pairs[] = $pairData;  // می‌توانید اینجا داده‌های مربوطه را به آرایه اضافه کنید
        //     }
        // }
        // var_dump($pairs);
        ...
        //     $baseCurrency = reset($parts); // ارز اول جفت ارز
        //     if ($baseCurrency === 'usdt') {
        //         $pairs[] = $pair;
        //     }
        // }

        // فیلتر کردن جفت ارزهایی که USDT دارند
        // $pairs = [];
        // foreach ($pairsData as $pair => $data) {
        //     if (strpos('USDT', $pair) !== false) {
        //         $pairs[] = $pair;
        //     }
        // }
